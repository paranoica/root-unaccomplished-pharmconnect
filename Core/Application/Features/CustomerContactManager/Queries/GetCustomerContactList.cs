...
public class GetCustomerContactListProfile : Profile
{
    ...
    {
        CreateMap<CustomerContact, GetCustomerContactListDto>().ForMember(
...
public class GetCustomerContactListHandler : IRequestHandler<GetCustomerContactListRequest, GetCustomerContactListResult>
{
    ...
    public GetCustomerContactListHandler(IMapper mapper, IQueryContext context) => (_mapper, _context) = (mapper, context);
    ...
    {
        var query = _context.CustomerContact.AsNoTracking().ApplyIsDeletedFilter(request.IsDeleted).Include(x => x.Customer).AsQueryable();

        var entities = await query.ToListAsync(cancellationToken);
        var map_dtos = _mapper.Map<List<GetCustomerContactListDto>>(entities);
