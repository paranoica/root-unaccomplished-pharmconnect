...
public class GetCustomerContactByCustomerIdListProfile : Profile
{
    ...
    {
        CreateMap<CustomerContact, GetCustomerContactByCustomerIdListDto>().ForMember(
...
public class GetCustomerContactByCustomerIdListHandler : IRequestHandler<GetCustomerContactByCustomerIdListRequest, GetCustomerContactByCustomerIdListResult>
{
    ...
    public GetCustomerContactByCustomerIdListHandler(IMapper mapper, IQueryContext context) => (_mapper, _context) = (mapper, context);
    ...
    {
        var query = _context.CustomerContact.AsNoTracking().ApplyIsDeletedFilter(false).Include(x => x.Customer)
                    .Where(x => x.CustomerId == request.CustomerId).AsQueryable();

        var entities = await query.ToListAsync(cancellationToken);
        var map_dtos = _mapper.Map<List<GetCustomerContactByCustomerIdListDto>>(entities);
