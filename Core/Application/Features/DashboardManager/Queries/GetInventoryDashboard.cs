...
public class GetInventoryDashboardHandler : IRequestHandler<GetInventoryDashboardRequest, GetInventoryDashboardResult>
{
    ...
    public GetInventoryDashboardHandler(IQueryContext context) => _context = context;
    ...
    {
        var inventoryTransactionData = await _context.InventoryTransaction.AsNoTracking().ApplyIsDeletedFilter(false)
                                      .Include(x => x.Warehouse).Include(x => x.Product)
                                      .Include(x => x.WarehouseFrom).Include(x => x.WarehouseTo)
                                      .Where(x =>
        ...
        var inventoryStockData = _context.InventoryTransaction.AsNoTracking().ApplyIsDeletedFilter(false)
                                 .Include(x => x.Warehouse).Include(x => x.Product)
                                 .Where(x =>
                                     x.Status == InventoryTransactionStatus.Confirmed &&
                                     x.Warehouse!.SystemWarehouse == false &&
        ...
        var warehouseData = _context.Warehouse.AsNoTracking().ApplyIsDeletedFilter(false).Where(x => x.SystemWarehouse == false)
        ...
        var result = new GetInventoryDashboardResult
        {
            Data = new GetInventoryDashboardDto
            {
                ...
                InventoryStockDashboard = warehouseData.Select(wh => new BarSeries
                                                       {
                                                           Type = "Column",
                                                           XName = "x",
                                                           Width = 2,
