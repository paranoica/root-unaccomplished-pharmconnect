...
public class GetSalesDashboardHandler : IRequestHandler<GetSalesDashboardRequest, GetSalesDashboardResult>
{
    ...
    public GetSalesDashboardHandler(IQueryContext context) => _context = context;

    public async Task<GetSalesDashboardResult> Handle(GetSalesDashboardRequest request, CancellationToken cancellationToken)
    {
        var salesOrderItemData = await _context.SalesOrderItem.AsNoTracking().ApplyIsDeletedFilter(false)
                                .Include(x => x.SalesOrder).Include(x => x.Product)
        ...
        var salesByCustomerGroupData = _context.SalesOrderItem.AsNoTracking().ApplyIsDeletedFilter(false)
        ...
        var salesByCustomerCategoryData = _context.SalesOrderItem.AsNoTracking().ApplyIsDeletedFilter(false)
        ...
        var result = new GetSalesDashboardResult
        {
            Data = new GetSalesDashboardDto
            {
                ...
                SalesByCustomerGroupDashboard = Enum.GetValues(typeof(SalesOrderStatus)).Cast<SalesOrderStatus>()
                                                .Select(status => new BarSeries
                                                {
                                                    ...
                                                    DataSource = salesByCustomerGroupData
                                                        .Where(x => x.Status == status).Select(x => new BarDataItem
                                                        {
                ...
                SalesByCustomerCategoryDashboard = Enum.GetValues(typeof(SalesOrderStatus)).Cast<SalesOrderStatus>()
                                                   .Select(status => new BarSeries
                                                   {
                                                       Type = "Bar",
                                                       XName = "x",
