...
public class GetPurchaseDashboardHandler : IRequestHandler<GetPurchaseDashboardRequest, GetPurchaseDashboardResult>
{
    ...
    public GetPurchaseDashboardHandler(IQueryContext context) => _context = context;
    ...
    {
        var purchaseOrderItemData = await _context.PurchaseOrderItem.AsNoTracking().ApplyIsDeletedFilter(false)
                                   .Include(x => x.PurchaseOrder).Include(x => x.Product)
                                   .Where(x => x.PurchaseOrder!.OrderStatus == PurchaseOrderStatus.Confirmed)
                                   .OrderByDescending(x => x.PurchaseOrder!.OrderDate).Take(30).ToListAsync(cancellationToken);

        var purchaseByVendorGroupData = _context.PurchaseOrderItem.AsNoTracking().ApplyIsDeletedFilter(false)
                                        .Include(x => x.PurchaseOrder).ThenInclude(x => x!.Vendor).ThenInclude(x => x!.VendorGroup)
                                        .Include(x => x.Product).Where(x => x.Product!.Physical == true)
                                        .Select(x => new
                                        {
        ...
        var purchaseByVendorCategoryDate = _context.PurchaseOrderItem.AsNoTracking().ApplyIsDeletedFilter(false)
                                           .Include(x => x.PurchaseOrder).ThenInclude(x => x!.Vendor)
                                           .ThenInclude(x => x!.VendorCategory).Include(x => x.Product)
        ...
        var result = new GetPurchaseDashboardResult
        {
            Data = new GetPurchaseDashboardDto
            {
                ...
                PurchaseByVendorGroupDashboard = Enum.GetValues(typeof(PurchaseOrderStatus)).Cast<PurchaseOrderStatus>()
                ...
                PurchaseByVendorCategoryDashboard = Enum.GetValues(typeof(PurchaseOrderStatus)).Cast<PurchaseOrderStatus>()
                                                   .Select(status => new BarSeries
                                                   {
                                                       Type = "Column",
                                                       XName = "x",
