...
public class GetDeliveryOrderSingleHandler : IRequestHandler<GetDeliveryOrderSingleRequest, GetDeliveryOrderSingleResult>
{
    ...
    public GetDeliveryOrderSingleHandler(IQueryContext context) => _context = context;
    ...
    {
        var queryData = _context.DeliveryOrder.AsNoTracking().Include(x => x.SalesOrder).ThenInclude(x => x.Customer)
                        .Where(x => x.Id == request.Id).AsQueryable();

        var data = await queryData.SingleOrDefaultAsync(cancellationToken);
        var queryTransactionList = _context.InventoryTransaction.AsNoTracking().ApplyIsDeletedFilter(false)
                                   .Include(x => x.Product).Include(x => x.Warehouse).Include(x => x.WarehouseFrom)
                                   .Include(x => x.WarehouseTo)
                                   .Where(x => x.ModuleId == request.Id && x.ModuleName == nameof(DeliveryOrder)).AsQueryable();
