...
public class GetDeliveryOrderListProfile : Profile
{
    ...
    {
        CreateMap<DeliveryOrder, GetDeliveryOrderListDto>().ForMember(
...
public class GetDeliveryOrderListHandler : IRequestHandler<GetDeliveryOrderListRequest, GetDeliveryOrderListResult>
{
    ...
    public GetDeliveryOrderListHandler(IMapper mapper, IQueryContext context) => (_mapper, _context) = (mapper, context);
    ...
    {
        var query = _context.DeliveryOrder.AsNoTracking().ApplyIsDeletedFilter(request.IsDeleted)
                    .Include(x => x.SalesOrder).AsQueryable();

        var entities = await query.ToListAsync(cancellationToken);
        var map_dtos = _mapper.Map<List<GetDeliveryOrderListDto>>(entities);
