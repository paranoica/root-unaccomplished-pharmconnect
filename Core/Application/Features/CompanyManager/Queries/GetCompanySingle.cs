...
public class GetCompanySingleHandler : IRequestHandler<GetCompanySingleRequest, GetCompanySingleResult>
{
    ...
    public GetCompanySingleHandler(IQueryContext context, IMapper mapper) => (_context, _mapper) = (context, mapper);
    public async Task<GetCompanySingleResult> Handle(GetCompanySingleRequest request, CancellationToken cancellationToken)
    {
        var query = _context.Company.AsNoTracking().AsQueryable().Where(x => x.Id == request.Id);
        //query = query.Where(x => x.Id == request.Id);

        var entity = await query.SingleOrDefaultAsync(cancellationToken);
        var map_dtos = _mapper.Map<GetCompanySingleDto>(entity);
