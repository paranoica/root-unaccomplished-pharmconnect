...
public class GetCustomerListProfile : Profile
{
    ...
    {
        CreateMap<Customer, GetCustomerListDto>().ForMember(
...
public class GetCustomerListHandler : IRequestHandler<GetCustomerListRequest, GetCustomerListResult>
{
    ...
    public GetCustomerListHandler(IMapper mapper, IQueryContext context) => (_mapper, _context) = (mapper, context);
    public async Task<GetCustomerListResult> Handle(GetCustomerListRequest request, CancellationToken cancellationToken)
    {
        var query = _context.Customer.AsNoTracking().ApplyIsDeletedFilter(request.IsDeleted).Include(x => x.CustomerGroup)
                    .Include(x => x.CustomerCategory).AsQueryable();

        var entities = await query.ToListAsync(cancellationToken);
        var map_dtos = _mapper.Map<List<GetCustomerListDto>>(entities);
